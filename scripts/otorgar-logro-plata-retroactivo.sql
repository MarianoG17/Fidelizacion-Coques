...
WITH logro_plata AS (
  ...
  WHERE tipo = 'NIVEL_ALCANZADO'
    AND "nivelId" IN (
      SELECT id FROM "Nivel" WHERE nombre = 'Plata'
    )
  LIMIT 1
),
...
clientes_sin_logro AS (
  ...
  WHERE c."nivelId" = (SELECT id FROM nivel_plata)
    AND NOT EXISTS (
      SELECT 1
...
INSERT INTO "LogroCliente" ("clienteId", "logroId", visto, "obtainedAt")
SELECT 
  csl."clienteId",
  (SELECT id FROM logro_plata),
  false,
  NOW()
...
FROM "Cliente" c
CROSS JOIN (SELECT id FROM "Logro" WHERE tipo = 'NIVEL_ALCANZADO' AND "nivelId" IN (SELECT id FROM "Nivel" WHERE nombre = 'Plata') LIMIT 1) lp
WHERE c."nivelId" = (SELECT id FROM "Nivel" WHERE nombre = 'Plata' LIMIT 1)
  AND NOT EXISTS (
    SELECT 1 FROM "LogroCliente" lc 
    WHERE lc."clienteId" = c.id AND lc."logroId" = lp.id
  );
